...
class Relatorios extends MY_Controller
{
    ...
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'rDevedor')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de devedores.');
            redirect(base_url());
        }
        $this->data['view'] = 'relatorios/rel_devedores';
        return $this->layout();
    }
    ...
    public function devedoresCustom()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'rDevedor')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de devedores.');
            redirect(base_url());
        ...
        $data['devedores'] = $this->Relatorios_model->devedoresCustom($dataInicial, $dataFinal, $this->input->get('tipodevedor'));
        $data['emitente'] = $this->Samrd_model->getEmitente();
        $data['title'] = 'Relatório de Devedores Customizado';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirDevedores', $data, true);
        pdf_create($html, 'relatorio_devedores' . date('d/m/y'), true);
    }

    public function devedoresRapid()
    {
        if (!$this->permission->checkPermission($this->session->userdata('permissao'), 'rDevedor')) {
            $this->session->set_flashdata('error', 'Você não tem permissão para gerar relatórios de devedores.');
            redirect(base_url());
        ...
        if ($format == 'xls') {
            $devedores = $this->Relatorios_model->devedoresRapid($array = true);
            $cabecalho = [
                'Código' => 'integer',
                'Nome' => 'string',
                'Sexo' => 'string',
            ...
            foreach ($devedores as $devedor) {
                $writer->writeSheetRow('Sheet1', $devedor);
            }

            $arquivo = $writer->writeToString();
            $this->load->helper('download');
            force_download('relatorio_devedores.xlsx', $arquivo);
            return;
        }

        $data['devedores'] = $this->Relatorios_model->devedoresRapid();
        $data['emitente'] = $this->Samrd_model->getEmitente();
        $data['title'] = 'Relatório de Devedores';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);
    ...
    public function skuCustom()
    {
        ...
        $dataInicial = $this->input->get('dataInicial');
        $dataFinal = $this->input->get('dataFinal');
        $devedor = $this->input->get('devedores_id');
        $format = $this->input->get('format');
        $origem = $this->input->get('origem');

        if ($format == 'xls') {
            $vendas = $this->Relatorios_model->skuCustom($dataInicial, $dataFinal, $devedor, $origem, true);
        ...
        $data['resultados'] = $this->Relatorios_model->skuCustom($dataInicial, $dataFinal, $devedor, $origem);
        $data['emitente'] = $this->Samrd_model->getEmitente();
        $data['title'] = 'Relatório SKU';
        $data['topo'] = $this->load->view('relatorios/imprimir/imprimirTopo', $data, true);
    ...
    public function osCustom()
    {
        ...
        $dataInicial = $this->input->get('dataInicial');
        $dataFinal = $this->input->get('dataFinal');
        $devedor = $this->input->get('devedor');
        $responsavel = $this->input->get('responsavel');
        $status = $this->input->get('status');
        $format = $this->input->get('format');

        $isXls = $format === 'xls';
        $os = $this->Relatorios_model->osCustom($dataInicial, $dataFinal, $devedor, $responsavel, $status, $isXls);
        $totalProdutos = 0;
        $totalServicos = 0;
        foreach ($os as $o) {
            $totalProdutos += $isXls
        ...
        $emitente = $this->Samrd_model->getEmitente();
        ...
        if ($format == 'xls') {
            ...
            $lancamentosFormatados = array_map(function ($item) {
                return [
                    ...
                    'devedor_fornecedor' => $item['devedor_fornecedor'],
    ...
    public function vendasCustom()
    {
        ...
        $isXls = $format === 'xls';
        $vendas = $this->Relatorios_model->vendasCustom($dataInicial, $dataFinal, $devedor, $responsavel, $isXls);
        $totalVendas = 0;
        foreach ($vendas as $venda) {
            $totalVendas += $isXls
                ? floatval($venda['valorTotal'])

...
class Mine extends CI_Controller
{
    ...
    public function login()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $this->db->limit(1);
            $devedor = $this->db->get('devedores');
            if ($devedor->num_rows() > 0) {
                $devedor = $devedor->row();
                $dados = ['nome' => $devedor->nomeDevedor, 'devedor_id' => $devedor->idDevedores, 'email' => $devedor->email, 'conectado' => true, 'isDevedor' => true];
    ...
    public function painel()
    {
        ...
        $data['compras'] = $this->Conecte_model->getLastCompras($this->session->userdata('devedor_id'));
        $data['os'] = $this->Conecte_model->getLastOs($this->session->userdata('devedor_id'));
    ...
    public function editarDados($id = null)
    {
        ...
        if ($this->form_validation->run('devedores') == false) {
            ...
            if ($this->Conecte_model->edit('devedores', $data, 'idDevedores', $this->input->post('idDevedores')) == true) {
    ...
    public function compras()
    {
        ...
        $config['total_rows'] = $this->Conecte_model->count('vendas', $this->session->userdata('devedor_id'));
        $config['per_page'] = 10;
        ...
        $data['results'] = $this->Conecte_model->getCompras('vendas', '*', '', $config['per_page'], $this->uri->segment(3), '', '', $this->session->userdata('devedor_id'));
    ...
    public function cobrancas()
    {
        ...
        $config['total_rows'] = $this->Conecte_model->count('cobrancas', $this->session->userdata('devedor_id'));
        $config['per_page'] = 10;
        $config['next_link'] = 'Próxima';
        $config['prev_link'] = 'Anterior';
        $config['full_tag_open'] = '<div class="pagination alternate"><ul>';
        ...
        $data['results'] = $this->Conecte_model->getCobrancas('cobrancas', '*', '', $config['per_page'], $this->uri->segment(3), '', '', $this->session->userdata('devedor_id'));
    ...
    public function os()
    {
        ...
        $config['total_rows'] = $this->Conecte_model->count('os', $this->session->userdata('devedor_id'));
        $config['per_page'] = 10;
        $config['next_link'] = 'Próxima';
        $config['prev_link'] = 'Anterior';
        $config['full_tag_open'] = '<div class="pagination alternate"><ul>';
        ...
        $data['results'] = $this->Conecte_model->getOs('os', '*', '', $config['per_page'], $this->uri->segment(3), '', '', $this->session->userdata('devedor_id'));
    ...
    public function visualizarOs($id = null)
    {
        ...
        $data['emitente'] = $this->samrd_model->getEmitente();

        if ($data['result']->idDevedores != $this->session->userdata('devedor_id')) {
            $this->session->set_flashdata('error', 'Esta OS não pertence ao devedor logado.');
    ...
    {
        $json = ['code' => 4001, 'error' => 'Erro interno' , 'errorDescription' => 'Cobrança não pode ser gerada pelo lado do devedor'];
    ...
    public function visualizarCompra($id = null)
    {
        ...
        if ($data['result']->devedores_id != $this->session->userdata('devedor_id')) {
            $this->session->set_flashdata('error', 'Esta OS não pertence ao devedor logado.');
            redirect('mine/painel');
        }
    ...
    public function adicionarOs()
    {
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data = [
                ...
                'devedores_id' => $this->session->userdata('devedor_id'), //set_value('idDevedor'),
            ...
            if (is_numeric($id = $this->Conecte_model->add('os', $data, true))) {
                ...
                $this->enviarOsPorEmail($idOs, $remetentes, 'Nova Ordem de Serviço #'.$idOs.' - Criada pelo Devedor');
    ...
    public function detalhesOs($id = null)
    {
        if (is_numeric($id) && $id != null) {
            ...
            if ($this->data['result']->idDevedores != $this->session->userdata('devedor_id')) {
                $this->session->set_flashdata('error', 'Esta OS não pertence ao devedor logado.');
                redirect('mine/painel');
            }
    ...
    // método para devedores se cadastratem
    public function cadastrar()
    {
        $this->load->model('devedores_model', '', true);
        $this->load->library('form_validation');
        $this->data['custom_error'] = '';
        $id = 0;
        ...
        if ($this->form_validation->run('devedores') == false) {
            ...
            $id = $this->devedores_model->add('devedores', $data);

            if ($id > 0) {
                $this->enviarEmailBoasVindas($id);
                $this->enviarEmailTecnicoNotificaDevedorNovo($id);
    ...
    private function enviarOsPorEmail($idOs, $remetentes, $assunto)
    {
        ...
        $dados['emitente'] = $this->samrd_model->getEmitente();
    ...
    private function enviarEmailBoasVindas($id)
    {
        ...
        $dados['emitente'] = $this->samrd_model->getEmitente();
        $dados['devedor'] = $this->devedores_model->getById($id);

        $emitente = $dados['emitente'][0]->email;
        $emitenteNome = $dados['emitente'][0]->nome;
        $remetente = $dados['devedor']->email;
        $assunto = 'Bem-vindo!';

        $html = $this->load->view('os/emails/devedornovo', $dados, true);
    ...
    private function enviarEmailTecnicoNotificaDevedorNovo($id)
    {
        $dados = [];
        $this->load->model('samrd_model');
        $this->load->model('devedores_model', '', true);
        ...
        $emitente = $dados['emitente'][0]->email;
        $emitenteNome = $dados['emitente'][0]->nome;
        $assunto = 'Novo Devedor Cadastrado no Sistema';
        ...
        foreach ($usuarios as $usuario) {
            ...
            $html = $this->load->view('os/emails/devedornovonotifica', $dados, true);
            $headers = [
                'From' => "\"$emitenteNome\" <$emitente>",
                'Subject' => $assunto,
                'Return-Path' => ''

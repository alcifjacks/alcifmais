<?php
class Samrd_model extends CI_Model
{
    ...
    public function pesquisar($termo)
    {
        $data = [];
        // buscando devedores
        $this->db->like('nomeDevedor', $termo);
        $this->db->or_like('telefone', $termo);
        $this->db->or_like('celular', $termo);
        $this->db->limit(15);
        $data['devedores'] = $this->db->get('devedores')->result();
    ...
    {
        $this->db->select('os.*, devedores.nomeDevedor');
        $this->db->from('os');

...
class Cobrancas_model extends CI_Model
{
    ...
    {
        $this->db->select('cobrancas.*, devedores.*');
        $this->db->from('cobrancas');
        $this->db->where('cobrancas.idCobranca', $id);
        $this->db->join('devedores', 'devedores.idDevedores = cobrancas.devedores_id');
        $this->db->limit(1);
    ...
    {
        return $this->db->query("SELECT DISTINCT `cobrancas`.*,`devedores`.*,`os`.* FROM `cobrancas`,`devedores`,`os` WHERE `charge_id` = $id AND `os`.`idOs` = `cobrancas`.`os_id`")->row();
    }

    public function getByVendas($id)
    {
        return $this->db->query("SELECT DISTINCT `cobrancas`.*,`devedores`.*,`vendas`.* FROM `cobrancas`,`devedores`,`vendas` WHERE `charge_id` = $id AND `vendas`.`idVendas` = `cobrancas`.`vendas_id`")->row();
    }

...
class Relatorios_model extends CI_Model
{
    ...
    public function devedoresCustom($dataInicial = null, $dataFinal = null, $tipo = null)
    {
        $whereData = '';
        if ($dataInicial != null) {
        ...
        $query = "SELECT * FROM devedores WHERE dataCadastro $whereData ORDER BY nomeDevedor";

        return $this->db->query($query, [$dataInicial, $dataFinal])->result();
    }

    public function devedoresRapid($array = false)
    {
        $this->db->order_by('nomeDevedor', 'asc');

        $result = $this->db->get('devedores');
        if ($array) {
    ...
    {
        $this->db->select('devedores.idDevedores, devedores.nomeDevedor, produtos.idProdutos, produtos.descricao, itens_de_vendas.quantidade, vendas.idVendas as idRelacionado, vendas.dataVenda as dataOcorrencia, itens_de_vendas.preco, (itens_de_vendas.preco * itens_de_vendas.quantidade) as precoTotal, "venda" as origem');
        $this->db->from('vendas');
        $this->db->join('itens_de_vendas', 'vendas.idVendas = itens_de_vendas.vendas_id');
        $this->db->join('devedores', 'devedores.idDevedores = vendas.devedores_id');
        $this->db->join('produtos', 'produtos.idProdutos = itens_de_vendas.produtos_id');
        $subQuery1 = $this->db->get_compiled_select();
        $this->db->reset_query();

        $this->db->select('devedores.idDevedores, devedores.nomeDevedor, produtos.idProdutos, produtos.descricao, produtos_os.quantidade, os.idOs as idRelacionado, os.dataInicial as dataOcorrencia, produtos_os.preco , (produtos_os.preco * produtos_os.quantidade) as precoTotal, "os" as origem');
        $this->db->from('os');
    ...
    public function skuCustom($dataInicial = null, $dataFinal = null, $devedor = null, $origem = null, $array = false)
    {
        $this->db->select('devedores.idDevedores, devedores.nomeDevedor, produtos.idProdutos, produtos.descricao, itens_de_vendas.quantidade, vendas.idVendas as idRelacionado, vendas.dataVenda as dataOcorrencia, itens_de_vendas.preco, (itens_de_vendas.preco * itens_de_vendas.quantidade) as precoTotal, "venda" as origem');
        $this->db->from('vendas');
        $this->db->join('itens_de_vendas', 'vendas.idVendas = itens_de_vendas.vendas_id');
        ...
        if ($devedor) {
            $this->db->where('idDevedores =', $devedor);
        }
    ...
    public function osRapid($array = false)
    {
        ...
        $this->db->select('os.*,devedores.nomeDevedor, total_servicos.total_servico, total_produtos.total_produto');
        $this->db->from('os');
        $this->db->join('devedores', 'devedores.idDevedores = os.devedores_id');
    ...
    public function osCustom($dataInicial = null, $dataFinal = null, $devedor = null, $responsavel = null, $status = null, $array = false)
    {
        $whereData = "";
        $whereDevedor = "";
        $whereResponsavel = "";
        ...
        if ($devedor != null) {
            $whereDevedor = "AND devedores_id = " . $this->db->escape($devedor);
        }
        if ($responsavel != null) {
        ...
        $query = "SELECT os.*,devedores.nomeDevedor, total_servicos.total_servico, total_produtos.total_produto FROM os
                   LEFT JOIN total_produtos ON total_produtos.os_id = os.idOs
                   LEFT JOIN total_servicos ON total_servicos.os_id = os.idOs
                   LEFT JOIN devedores ON os.devedores_id = devedores.idDevedores
                   WHERE idOs != 0 $whereData $whereDevedor $whereResponsavel $whereStatus
    ...
    {
        $this->db->select('vendas.*,devedores.nomeDevedor, usuarios.nome');
        $this->db->from('vendas');
        $this->db->join('devedores', 'devedores.idDevedores = vendas.devedores_id');
    ...
    public function vendasCustom($dataInicial = null, $dataFinal = null, $devedor = null, $responsavel = null, $array = false)
    {
        $whereData = "";
        $whereDevedor = "";
        $whereResponsavel = "";
        ...
        $query = "SELECT vendas.*,devedores.nomeDevedor, usuarios.nome FROM vendas
        LEFT JOIN devedores ON vendas.devedores_id = devedores.idDevedores
        LEFT JOIN usuarios ON vendas.usuarios_id = usuarios.idUsuarios
        WHERE idVendas != 0 $whereData $whereDevedor $whereResponsavel ORDER BY vendas.idVendas";

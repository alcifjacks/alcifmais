            ...
            <div class="widget-box" style="min-height: 200px">
                ...
                <div class="widget-content nopadding tab-content">
                    <table class="table table-bordered ">
                        ...
                        <?php
                        if ($devedores == null) {
                            echo '<tr><td colspan="4">Nenhum devedor foi encontrado.</td></tr>';
                        }
                        foreach ($devedores as $r) {
                            echo '<tr>';
                            echo '<td>' . $r->idDevedores . '</td>';
                            echo '<td>' . $r->nomeDevedor . '</td>';
                            echo '<td>' . $r->documento . '</td>';
                            $cor = ($r->fornecedor ? '#CDB380' : '#CD0000');
                            echo '<td><span class="badge" style="background-color: ' . $cor . '; border-color: ' . $cor . '">' . ($r->fornecedor ? 'Fornecedor' : 'Devedor') . '</span> </td>';
                            echo '<td>';
                            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'vDevedor')) {
                                echo '<a style="margin-right: 1%" href="' . base_url() . 'index.php/devedores/visualizar/' . $r->idDevedores . '" class="btn tip-top" title="Ver mais detalhes"><i class="fas fa-eye"></i></a>';
                            }
                            if ($this->permission->checkPermission($this->session->userdata('permissao'), 'eDevedor')) {
                                echo '<a href="' . base_url() . 'index.php/devedores/editar/' . $r->idDevedores . '" class="btn btn-info tip-top" title="Editar Devedor"><i class="fas fa-edit"></i></a>';
                            }
                            echo '</td>';
                            echo '</tr>';
                        }
